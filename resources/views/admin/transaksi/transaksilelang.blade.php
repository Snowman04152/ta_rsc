...
@section('content')
    <div class="col py-3 pe-5 ">
        <div class="h2 p-2 "><b>Transaksi Lelang</b></div>
                ...
                <tbody>
                    @foreach ($transaksi as $transaksis)
                        <tr>
                            ...
                            <td>{{ $transaksis->penawaran->user->username }}</td>
                            <td>{{ $transaksis->penawaran->produk_lelang->nama_produk_lelang }}</td>
                            <td>{{ 'Rp. '.number_format($transaksis->penawaran->penawaran_harga) }}</td>
                            ...
                            @elseif($transaksis->status == 1)
                                <td> <span class="badge text-secondary p-2" style="background-color: #D9EFBD"> Sedang
                                        diproses</span></td>
                            @elseif($transaksis->status == 2)
                                <td><span class="badge text-secondary p-2" style="background-color: #A3D95D">Belum
                                        Diambil</span></td>
                            @elseif($transaksis->status == 3)
                                <td><span class="badge text-secondary p-2" style="background-color: #A3D95D">Dalam
                                        Perjalanan</span></td>
                            @elseif($transaksis->status == 4)
                                <td><span class="badge  p-2" style="background-color: #598420">Selesai</span></td>
                            @else
                                <td><span class="badge text-danger p-2" style="background-color: #FFDCDC">Dibatalkan</span>
                                </td>
                            @endif
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li><a class="dropdown-item"
                                                href="{{ route('transaksi.lelang.detail', ['id' => $transaksis->id]) }}">Detail</a>
                                        </li>
                                    </ul>
                                </div>
                            </td>
