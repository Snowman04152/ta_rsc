    ...
    <div class="col py-3 pe-5">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb fw-bold mb-4">
                <li class="breadcrumb-item "><a class="h5 text-decoration-none text-dark"
                        href="{{ route('transaksi.lelang.index') }}"><i class="bi bi-chevron-left fw-bold"></i>Detail Pesanan</a>
                </li>
            </ol>
        </nav>
        ...
        <div class="card rounded-4 me-5 fw-bold">
            <div class="row ms-3 mt-3">
                ...
                <div class="col-3">{{ $transaksi->penawaran->user->nama_lengkap }}</div>
            </div>
            <div class="row ms-3 ">
                <div class="col-9 text-secondary">No Telepon</div>
                <div class="col-3">{{ $transaksi->penawaran->user->telp_user }}</div>
            </div>
            <div class="row ms-3">
                <div class="col-9 text-secondary">Username</div>
                <div class="col-3">{{ $transaksi->penawaran->user->username }}</div>
            </div>
            <div class="row ms-3 mb-3">
                <div class="col-9 text-secondary">Email</div>
                <div class="col-3">{{ $transaksi->penawaran->user->email }}</div>
            </div>
        </div>
                    ...
                    @if ($transaksi->kurir == 'AMBIL_DITEMPAT')
                        <div class="row">
                            <div class="col-11  ">
                                ...
                                <div>Ambil di {{ $transaksi->penawaran->produk_lelang->lokasi_pengambilan }} pada
                                    {{ toIndoDate($transaksi->penawaran->produk_lelang->tanggal_pengambilan) }}</div>
                            </div>
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-12 col-md-6 col-lg-1 ">
                            <img src="{{ asset('storage/files/' . $transaksi->penawaran->produk_lelang->encrypted_filename) }}"
                                alt=".." class="img-fluid rounded w-100">
                        </div>
                        <div class="col-12 col-md-6 col-lg-11 ">
                            <div class="h5 fw-bold">{{ $transaksi->penawaran->produk_lelang->nama_produk_lelang }}</div>
                            <div class="d-flex justify-content-between">
                                <h5 class=" fw-bold" style="color:#598420 ;">
                                    {{ 'Rp.' . number_format($transaksi->penawaran->penawaran_harga) }}</h5>
                                <div>x1 Barang </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="card rounded-4 me-5 fw-bold">
            <div class="row ms-3 mt-3">
                ...
                <div class="col-3">{{ 'Rp.' . number_format($transaksi->penawaran->penawaran_harga) }}</div>
            </div>
            <div class="row ms-3">
            ...
            @if ($transaksi->kurir == 'AMBIL_DITEMPAT')
                @if ($transaksi->status == 1 || $transaksi->status == 0)
                    <div class="d-flex gap-5 justify-content-between">
                        <a href="{{ route('cancel.lelang', ['id' => $transaksi->id]) }}"
                            class=" btn btn-outline-greenlight fw-bold w-50" style="color:#598420 ;" type="button">Batalkan
                            Pesanan</a>
                        <a href="{{ route('confirm.lelang', ['id' => $transaksi->id]) }}" class="btn btn-greenlight fw-bold  w-50"
                            style="color:#598420 ;" type="button">Konfirmasi</a>
                    </div>
                @elseif ($transaksi->status == 2)
                    <div class="d-flex gap-5 justify-content-between">
                        <a href="{{ route('end.lelang', ['id' => $transaksi->id]) }}"
                            class=" btn btn-outline-greenlight fw-bold w-50" style="color:#598420 ;" type="button">Telah
                            Diambil</a>
                        <a href="" class="btn btn-greenlight fw-bold  w-50" style="color:#598420 ;"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div>
                        <form action="{{ route('addResi.lelang', ['id' => $transaksi->id]) }}" method="POST">
                            @csrf
